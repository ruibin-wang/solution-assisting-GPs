...
# write_to_json("disease_cos_dict.json", disease_cos_dict)

disease_cos_dict = read_json("disease_cos_dict.json")
#
# for key in disease_cos_dict:
#
#     y = np.array(disease_cos_dict[key])
#     x = np.array(range(0, len(disease_cos_dict[key])))
#     color = np.random.rand(3, )# 随机颜色
#     plt.plot(x,y, color=color, label=key)
#
# plt.legend(loc='lower left', ncol=5, fontsize='small')
# plt.xlabel("Augmented referral letters")
# plt.ylabel("Cosine distance to real referral letters")
# plt.show()
print(11)

plt.boxplot([np.array(disease_cos_dict[key]) for key in disease_cos_dict], labels=(disease_cos_dict.keys()), patch_artist=True)
plt.show()
